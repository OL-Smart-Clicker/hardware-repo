...
public:
	...
	static void    startAsAnchorCustom(uint16_t deviceAddress, uint16_t networkId, const byte mode[]);
	static void    startAsTag(char address[], const byte mode[], const bool randomShortAddress = true);
	static void    startAsTagCustom(uint16_t deviceAddress, uint16_t networkId, const byte mode[]);
