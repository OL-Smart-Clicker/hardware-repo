...
void DW1000RangingClass::startAsAnchorCustom(uint16_t deviceAddress, uint16_t networkId, const byte mode[]) {
    /// Set short address (2 bytes)
    _currentShortAddress[0] = deviceAddress & 0xFF;        // Low byte
    _currentShortAddress[1] = (deviceAddress >> 8) & 0xFF; // High byte

    // Generate EUI string: XX:XX:YY:YY:00:00:00:00 (deviceAddress, networkId, padding)
    char euiStr[24];
    snprintf(euiStr, sizeof(euiStr), "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
        _currentShortAddress[0], _currentShortAddress[1],
        networkId & 0xFF, (networkId >> 8) & 0xFF,
        0x00, 0x00, 0x00, 0x00);

    // Set EUI on DW1000 chip
    DW1000.convertToByte(euiStr, _currentAddress);
    DW1000.setEUI(euiStr);
    Serial.print("device EUI: ");
    Serial.println(euiStr);

    // Configure the network
    configureNetwork(deviceAddress, networkId, mode);
...
void DW1000RangingClass::startAsTagCustom(uint16_t deviceAddress, uint16_t networkId, const byte mode[]) {
    // Set short address (2 bytes)
    _currentShortAddress[0] = deviceAddress & 0xFF;        // Low byte
    _currentShortAddress[1] = (deviceAddress >> 8) & 0xFF; // High byte
